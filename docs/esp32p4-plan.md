...
### 2026-09-05: 実用スループット ― 原稿用紙 100 枚を 22 秒

実用の単位は「1 文字あたり何 µs か」ではなく「原稿何枚をどれだけで点字にできるか」なので、
実文を通しで測る `:feed` / `:tp` を追加した（`:feed <行>` でバッファに溜め、`:tp [N]` が
**1 行ずつ** predict → BrailleTranslator まで通して字/秒を出す。1 行 = 1 段落想定）。

測定: `dataset/basic_raw.tsv` の原文を約 400 字（原稿用紙 1 枚）ずつに束ねた 135 行・
**55,675 字**（≒ 139 枚）を実機で通し変換。

| 項目 | 実測 |
|---|---|
| 所要 | **30.66 秒** |
| スループット | **1,816 字/秒** |
| 内訳 | predict 509 µs/字 + 点字変換 42 µs/字 = 551 µs/字 |
| **原稿用紙 100 枚（4 万字）換算** | **22.0 秒** |
| ↑ この単位の意味 | 原稿用紙 100 枚 ≒ **点字の本 1 冊分**。「1 冊を何秒で点訳できるか」が実用の単位 |
| 起動（電源投入 → 変換可能） | **1.2 秒**（初期化 50ms + 点字テーブル 132ms + モデル展開 1,002ms） |

「1 秒 1,000 字を割ると実用上つらい」という基準に対し **1.8 倍の余裕**。

1 文字あたりのコストが `:phases`（190 字の 1 行）の 437 µs より大きいのは、1 行あたりの
固定費と点字変換 42 µs が乗るため。**行が短いほど不利**で、42 字/行の文セットでは
577 µs/字 = 1,732 字/秒 だった（同じ本文でも段落に束ねると 1,816 字/秒）。

### 2026-09-05: PSRAM 周りで試して却下した 2 つ

| 設定 | 結果 | 備考 |
|---|---|---|
| `CONFIG_SPIRAM_MODE_HEX` / `SPEED_200M` | **既に最速**（変更不要） | P4 の既定で HEX（16 ビット幅）200MHz |
| `CONFIG_SPIRAM_XIP_FROM_PSRAM=y` | **却下**（4 万字 22.0 → 23.2 秒） | 下記 |
| `CONFIG_SPIRAM_MEMTEST=n` | **効果なし**（main 到達 50ms で不変） | 既定のまま |

`SPIRAM_XIP_FROM_PSRAM`（アプリの `.text`/`.rodata` をフラッシュではなく PSRAM から実行）は、
ライン充填コストが フラッシュ QIO 3.2µs > PSRAM 1µs である以上「速くなるはず」だったが、
**実測は悪化した**（predict 508→524 µs/字、点字変換 42→56 µs/字、PSRAM 常駐 +1MB）。
`:phases` を見ると normalize 3.1→1.5・source 6.3→4.0 とコード主体の段は確かに速くなっており、
命令フェッチ単体は速い。それでも総計が負けるのは、**命令フェッチが、モデルを読む
データアクセスと同じ PSRAM バスを奪い合う**から。フラッシュに置いたままなら、
命令フェッチ（SPI フラッシュ）とデータ（PSRAM）が**別バスで並行**できる。

→ 教訓: **速い方のメモリに全部載せるのが最適とは限らない**。QIO 化が効いたのは
フラッシュ側のバスを太くして、並行性を保ったままミスを安くしたから。XIP-from-PSRAM は
その並行性を捨てる方向だった。この構図は借用ロードの評価にも効く ── 借用ロードは逆に
**データの一部をフラッシュ側バスへ逃がす**方向なので、PSRAM バスが飽和している局面では
`:xip` の比（語彙 5.46x）ほど不利ではない可能性がある（未検証。再挑戦時の観点として残す）。
