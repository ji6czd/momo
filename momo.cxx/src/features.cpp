...
static bool is_kurai_char(char32_t cp) {
    // 十百千万億兆
    return cp == U'十' || cp == U'百' || cp == U'千'
        || cp == U'万' || cp == U'億' || cp == U'兆';
}

// 小書きかな（ひらがな・カタカナ）
static bool is_small_kana(char32_t cp) {
    // ぁぃぅぇぉゃゅょゎ
    if (cp == U'ぁ' || cp == U'ぃ' || cp == U'ぅ' || cp == U'ぇ' || cp == U'ぉ' ||
        cp == U'ゃ' || cp == U'ゅ' || cp == U'ょ' || cp == U'ゎ') return true;
    // ァィゥェォャュョヮヵヶ
    if (cp == U'ァ' || cp == U'ィ' || cp == U'ゥ' || cp == U'ェ' || cp == U'ォ' ||
        cp == U'ャ' || cp == U'ュ' || cp == U'ョ' || cp == U'ヮ' ||
        cp == U'ヵ' || cp == U'ヶ') return true;
    return false;
}

static bool is_ascii_printable(char32_t cp) {
    return cp >= 0x21 && cp <= 0x7E;
}

static bool is_ascii_space(char32_t cp) {
    return cp == U' ' || cp == U'\t';
}
...
static bool try_match_bracket(
    const std::u32string& text, size_t pos,
    std::u32string& out, size_t& next_pos)
{
...
static bool try_match_kana_block(
    const std::u32string& text, size_t pos,
    std::u32string& out, size_t& next_pos)
{
...
/// 先頭・末尾はスペース/タブでないこと。
static bool try_match_ascii_block(
    const std::u32string& text, size_t pos,
    std::u32string& out, size_t& next_pos)
{
    ...
    size_t end = pos + 1;
    while (end < text.size() &&
           (is_ascii_printable(text[end]) || is_ascii_space(text[end]))) {
        ++end;
    }
    // 末尾のスペース/タブを切り詰める
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (source_seq[i].ctype == CharType::KANJI) {
            ...
            if (i > 0 && source_seq[i-1].ctype == CharType::JAPANESE_NUMERIC) {
                int num_run = 0;
                for (size_t j = i - 1; j < n && source_seq[j].ctype == CharType::JAPANESE_NUMERIC; --j)
                    ++num_run;
