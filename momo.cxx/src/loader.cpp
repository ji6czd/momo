...
static uint8_t read_u8(std::FILE* fp) {
  uint8_t v;
  read_exact(fp, &v, 1, "u8");
  return v;
}
static uint32_t read_u32(std::FILE* fp) {
  uint32_t v;
  read_exact(fp, &v, 4, "u32");
  return v;
}
static float read_f32(std::FILE* fp) {
  float v;
  read_exact(fp, &v, 4, "f32");
  return v;
}
...
static void csr_to_csc(uint32_t n_classes, uint32_t n_features, const std::vector<uint32_t>& csr_indptr,
                       const std::vector<uint32_t>& csr_indices, const std::vector<int8_t>& csr_data,
                       std::vector<uint32_t>& csc_colptr, std::vector<uint32_t>& csc_rowind,
...
MomoModel load_model(const std::string& path) {
  ...
  read_exact(fp, csr_indptr.data(), (model.n_classes + 1) * sizeof(uint32_t), "csr_indptr");

  std::vector<uint32_t> csr_indices(n_nonzero);
  read_exact(fp, csr_indices.data(), n_nonzero * sizeof(uint32_t), "csr_indices");

  std::vector<int8_t> csr_data(n_nonzero);
  read_exact(fp, csr_data.data(), n_nonzero * sizeof(int8_t), "csr_data");

  csr_to_csc(model.n_classes, model.n_features, csr_indptr, csr_indices, csr_data, model.csc_colptr, model.csc_rowind,
             model.csc_data);

  // --- 読みモデル intercept ---
  model.intercept_read.resize(model.n_classes);
  read_exact(fp, model.intercept_read.data(), model.n_classes * sizeof(float), "intercept_read");
