...
Predictor::Predictor(const PredictorConfig& config) : _config(config) {
    _load_tagger(config.path_read, &_model_read, &_tagger_read);

    if (!config.path_boundary.empty()) {
        _load_tagger(config.path_boundary, &_model_boundary, &_tagger_boundary);
    }
...
Predictor::~Predictor() {
    if (_tagger_read)    { _tagger_read->release(_tagger_read);       }
    if (_model_read)     { _model_read->release(_model_read);         }
    if (_tagger_boundary){ _tagger_boundary->release(_tagger_boundary); }
    if (_model_boundary) { _model_boundary->release(_model_boundary); }
}
...
void Predictor::_load_tagger(const std::string& path,
                              crfsuite_model_t**  out_model,
                              crfsuite_tagger_t** out_tagger) const {
    int ret = crfsuite_create_instance_from_file(path.c_str(), (void**)out_model);
    if (ret != 0 || *out_model == nullptr) {
        throw std::runtime_error("モデルファイルを開けませんでした: " + path);
    }
    ret = (*out_model)->get_tagger(*out_model, out_tagger);
    if (ret != 0 || *out_tagger == nullptr) {
        (*out_model)->release(*out_model);
        *out_model = nullptr;
        throw std::runtime_error("taggerの取得に失敗しました: " + path);
    }
}
    ...
    while (std::getline(f, line)) {
        ...
        if (t == std::string::npos) {
            throw std::runtime_error(
                "カスタム辞書 " + path + " の " + std::to_string(lineno) +
...
PredictionResult Predictor::predict(const std::string& utf8_text) const {
    if (utf8_text.empty()) {
    ...
    auto refined = _refine(pre.source_seq, raw_labels, raw_confidences, boundary_labels,
                           pre.bypass_indices, pre.ascii_overrides, pre.dict_overrides);
...
Predictor::PreprocessResult Predictor::_preprocess(const std::u32string& text) const {
    ...
    for (const auto& unit : units) {
        ...
        if (unit.ctype == CharType::ALPHA || unit.ctype == CharType::NUM) {
            ...
            if (unit.text.empty() || unit.text.front() == U' ' || unit.text.front() == U'\t' ||
                unit.text.back()  == U' ' || unit.text.back()  == U'\t') {
        ...
        for (size_t i = 0; i < unit.text.size(); ++i) {
            result.source_seq.push_back({
                std::u32string(1, unit.text[i]),
                unit.orig_idx + static_cast<int>(i),
                unit.ctype
            });
            if (is_ascii_bypass) {
    ...
    if (!_dict_index.empty()) {
        ...
        while (i < n) {
            if (result.bypass_indices.count(i)) { ++i; continue; }
...
void Predictor::_run_inference(const std::vector<SourceUnit>& source_seq,
    ...
    auto make_items = [&](std::vector<std::string>& labels_out,
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (const auto& [feat, val] : features[i]) {
                int aid = attrs->to_id(attrs, feat.c_str());
                if (aid >= 0) {
...
Predictor::RefineResult Predictor::_refine(
    const std::vector<SourceUnit>&           source_seq,
...
PredictionResult Predictor::_assemble(
...
std::string Predictor::_kurai_fallback(char32_t ch,
                                        char32_t left_char,
                                        CharType  left_ctype,
...
Predictor::_find_longest_match(const std::vector<SourceUnit>& source_seq, int pos) const {
    ...
    for (const auto& [surface, readings] : it->second) {
        ...
        bool match = true;
        for (int j = 0; j < len; ++j) {
            if (source_seq[pos + j].text.empty() ||
                source_seq[pos + j].text[0] != surface[j]) {
                match = false;
                break;
            }
        }
