...
struct PredictorConfig {
    ...
    std::string path_fallback_dict;  ///< フォールバック辞書TSVのパス
...
private:
    ...
    std::unordered_map<char32_t, FallbackEntry> _fallback_dict;
    ...
    RefineResult _refine(const std::vector<SourceUnit>&           source_seq,
    ...
    PredictionResult _assemble(const std::string&              utf8_text,
    ...
    // --- フォールバックロジック ---
    std::string _kurai_fallback(char32_t ch,
                                char32_t left_char,
                                CharType left_ctype,
    ...
    _find_longest_match(const std::vector<SourceUnit>& source_seq, int pos) const;
};
