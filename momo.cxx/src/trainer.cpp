...
static void split_labels(const std::vector<std::string>& raw_labels, std::vector<std::string>& y_read,
...
void Trainer::train(const std::string& tsv_path, bool transitions) {
  ...
  // --- 0. 詳細表示するTrainer ---
  class VerboseTrainer : public CRFSuite::Trainer {
   public:
    void message(const std::string& msg) override { std::cerr << msg; }
  };
  // --- 1. TSV読み込み ---
  ...
  // --- 4. 共通パラメータ設定 ---
  auto set_params = [&](VerboseTrainer& trainer) {
    trainer.set("c1", "0.1");
    trainer.set("c2", "0.01");
  ...
  std::cerr << "    zf.write('" << path_boundary << "', '" << base.substr(base.rfind('/') + 1)
            << "_boundary.crfsuite')\n";
  std::cerr << "    zf.write('" << json_path << "', 'version_info.json')\n";
  std::cerr << "\"\n";

  auto elapsed = std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - t_start).count();
  std::cerr << "\n✅ 学習完了 (合計 " << elapsed << " 秒)\n";
