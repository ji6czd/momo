...
static void decode_one(const uint8_t*& p, const uint8_t* end, char32_t& cp) {
  ...
  if ((b & 0xF0) == 0xE0) {
    ...
    cp = ((char32_t)(b & 0x0F) << 12) | ((char32_t)(p[1] & 0x3F) << 6) | (p[2] & 0x3F);
    if (cp < 0x800 || (cp >= 0xD800 && cp <= 0xDFFF)) {
      cp = 0xFFFD;
      ++p;
      return;  // 過長エンコード or サロゲート
    }
    p += 3;
    return;
  }
  ...
  if ((b & 0xF8) == 0xF0) {
    ...
    cp =
        ((char32_t)(b & 0x07) << 18) | ((char32_t)(p[1] & 0x3F) << 12) | ((char32_t)(p[2] & 0x3F) << 6) | (p[3] & 0x3F);
    if (cp < 0x10000 || cp > 0x10FFFF) {
      cp = 0xFFFD;
      ++p;
      return;  // 過長エンコード or 範囲外
    }
    p += 4;
    return;
  }
