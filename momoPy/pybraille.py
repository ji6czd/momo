    ...
    for index, c in enumerate(normalized_text):
        ...
        if ord(c) < 0x7F:
            ...
            if c.isalpha():
                ...
                # 数字の小数点、カンマである可能性
                # 数字フラグはリセットしない
                not inForeignWordFlag
            else:
                # その他の文字でも外国語フラグをリセット
                inForeignWordFlag = False
                inDigitFlag = False
            braille_str += jp_braille_table.get(c.lower(), " ")
        else:
            # 日本語文字
